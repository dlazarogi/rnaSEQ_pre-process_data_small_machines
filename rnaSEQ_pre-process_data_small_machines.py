...
# FASTQC can give problems with java version. use this alternative:
# wget https://www.bioinformatics.babraham.ac.uk/projects/fastqc/fastqc_v0.11.9.zip
# unzip fastqc_v0.11.9.zip
# chmod +x FastQC/fastqc
# and copy in /usr/local/bin the content of the folder where fastqc is already, not the upper level
...
# To generate the database indexes for sortmerna V2.0 from Github, because the newer versions use a different database grouped
# (directly copy the fasta files to the rRNA_databases folder and then generate the indexes)
...
# NOTE: CHANGE FROM VERSION 4.34 with the databases, probably I have to modify my script...
...
while len(drlist1) > 0:
        ...
        while len(drlist) > 0:
           ...
           day = time.strftime("%y/%m/%d")
           hour = time.strftime("%H:%M:%S")
           print("The sortmeRNA process starts at \033[0;94;48m"+ hour + "\033[0;0m on \033[0;94;48m" + day + '\033[0;0m')
        ...
        #print (lista_sortme). I need to improve this part
           cmd = (
           f'/usr/share/sortmerna/sortmerna '
		...
		f"STAR --runThreadN 4 "
		f"--genomeDir {genome} "
		f"--readFilesIn ./{first_file}_paired.fq.gz ./{second_file}_paired.fq.gz "
...
#Feature Counts with all .bam files generated -p for paired end bam files.
day = time.strftime("%y/%m/%d")
hour = time.strftime("%H:%M:%S")
print('Feature counts starts at \033[0;94;48m' + hour + '\033[0;0m on \033[0;94;48m' + day + '\033[0;0m')
p = subprocess.Popen('featureCounts -a '+ repr(feature)+' -p -o feautreCounts_results.csv ./STAR_results/*.bam -T '+ repr(threads) + '--verbose',
    shell=True, 
    stdout=subprocess.PIPE, 
    stderr=subprocess.PIPE)
out, err = p.communicate()
